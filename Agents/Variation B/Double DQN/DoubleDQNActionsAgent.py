import numpy as np
import torch as T
from NetworkActions import BoardDeepQnetwork as Network
...
class DoubleDQNAgent():
    def __init__(self, dims,all_actions,epsilon =0.9, gamma = 0.99,lr=0.001,
                 ...
                 algo='DoubleDQNActions',chkpt_dir='board/dqnTorch',env_name='FullBoard'):
    ...
    def choose_action(self,obs):
        ...
        if np.random.random() > self.epsilon:
            ...
            #action=self.action_dictionary_f[index]
        else:
            action = np.random.choice(self.n_actions)
        self.frames +=1
        #action=self.action_dictionary_f[action]
        return action
