...
class Tetris:
    ...
    def __init__(self,rew):
        self.reset()
        self.prev_props=self._get_board_props(self.board)
        self.rotations=[0,90,180,270]
        self.p0={(7,0):(6,0),(7,180):(6,180),(8,0):(6,0),(8,180):(6,180),(9,0):(6,0),(9,180):(6,180)}
        self.p1={(8,0):(7,0),(8,180):(7,180),(9,0):(7,0),(9,180):(7,180),(9,90):(8,90),(9,270):(8,270)}
        self.p2={(8,90):(7,90),(8,270):(7,270),(9,0):(8,0),(9,180):(8,180),(9,90):(7,90),(9,270):(7,270)}
        self.p3=self.p2
        self.p4=self.p1
        self.p5=self.p1
        self.p6={(9,0):(8,0),(9,90):(8,90),(9,180):(8,180),(9,270):(8,270)}
        self.inv_m={0:self.p0,1:self.p1,2:self.p2,3:self.p3,4:self.p4,5:self.p5,6:self.p6}
        if rew=='Reward_A':
        ...
        if rew=='Reward_CC':
            self.reward_fun=self.reward_function_CC
            self.over=self.terminal_f_CC
    ...
    def terminal_f_D(self):
        return -100
    def terminal_f_E(self):
        return -100
    def terminal_f_F(self):
        return -500
    def terminal_f_CC(self):
        return -100
    ...
    def reward_function_CC(self,lines_cleared):
        props=self._get_board_props(self.board)
        prev_p=self.prev_props.copy()
        lines=lines_cleared
        holes=props[1]-prev_p[1]
        bumpiness=props[2]-prev_p[2]
        sum_height=props[3]-prev_p[3]
    ...
    def play_g(self, x, rotation, render=False, render_delay=None):
        '''Makes a play given a position and a rotation, returning the reward and if the game is over, uses render_g'''
        p=self.current_piece
        temp=(x,rotation)
        if temp in self.inv_m[p].keys():
            x,rotation=self.inv_m[p][temp]
